    ...
    for col in df_final.columns:
        if col.endswith("_extracted"):
            ...
            summary["results"][entity_name] = {
                "total_extractions": sum(len(e) if isinstance(e, list) else 0 for e in extractions),
                "documents_with_extraction": sum(1 for e in extractions if e and len(e) > 0),
            ...
            if eval_cols:
                ...
                for eval_col in eval_cols:
                    if not eval_col.endswith("_extracted"):
                        ...
                        summary["results"][entity_name]["evaluations"][eval_name] = values
    
    with open("output/extraction_summary.json", "w") as f:
        json.dump(summary, f, indent=2)
    
    print("Done! Results saved to output/")
    print(f"  - Full results: output/results.csv")
...
def run_databricks(config):
    ...
    for entity_name in extraction_results.keys():
        ...
        count = df_extracted.filter(f"{col_name} is not null and {col_name} != '[]'").count()
