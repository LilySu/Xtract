...
class AzureOpenAIProvider(BaseProvider):
    ...
    def __init__(self, model_name: str, config: Dict[str, Any]):
        ...
        try:
            ...
            raise ImportError("Azure OpenAI is not available. Please install openai package or use OpenAI provider instead.")
...
class OpenAIProvider(BaseProvider):
    ...
    def infer(self, prompt: str, **kwargs) -> str:
        ...
        for key, value in kwargs.items():
            if key not in params and key not in ["deployment"]:  # Filter out Azure-specific params
                params[key] = value
...
class ModelRouter:
    ...
    def get_provider(self, model_name: str) -> BaseProvider:
        ...
        if not provider_name:
            provider_name = self.main_config.get("routing", {}).get("default_provider", "openai")
