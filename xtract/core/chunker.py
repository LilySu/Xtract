...
class Chunker:
    ...
    def _load_config(self, config_path: str) -> Dict[str, Any]:
        ...
        if not path.exists():
            ...
            return {"chunking": {"strategy": "page", "max_chars": 1000, "overlap": 100}}

        with open(path, "r") as f:
            return yaml.safe_load(f)

    def chunk(
        self, paragraphs: List[Dict], tables: Optional[List[Dict]] = None
    ) -> List[str]:
    ...
    def _chunk_by_page(
        ...
        if signature_pages and self.config.get("chunking", {}).get(
