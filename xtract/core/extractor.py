...
class Extractor:
    ...
    def _extract_pandas(
        ...
        for entity in entities:
            ...
            df[col_name] = df.apply(lambda row: self._process_row(row, entity), axis=1)
    ...
    def _progressive_match(self, extraction: Dict, text: str) -> Optional[Dict]:
        ...
        for pass_num in range(1, fuzzy_passes + 1):
            ...
            threshold = matching_config.get(
                threshold_key, matching_config.get("fuzzy_threshold", 0.75)
            )
    ...
    def _token_based_match(
        self, extract_tokens: List[str], source_tokens: List[str], threshold: float
    ) -> float:
