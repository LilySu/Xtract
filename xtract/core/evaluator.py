...
class Evaluator:
    def __init__(self, eval_dir: str = "config/evaluation", settings_path: str = "config/settings.yaml"):
    ...
    def _needs_llm(self) -> bool:
        ...
        llm_methods = ["context_complexity", "hallucination_detection", "completeness_check", 
                      "consistency_check", "relevance_score"]
        return any(cfg.get("method") in llm_methods for cfg in self.eval_configs.values())
    
    def evaluate(self, df: pd.DataFrame, extraction_results: Dict[str, List]) -> pd.DataFrame:
    ...
    def _evaluate_extraction(self, row: Union[pd.Series, Dict], extractions: Any, config: Dict, entity_name: str) -> Any:
        ...
        elif self.model and method in ["context_complexity", "hallucination_detection", 
                                       "completeness_check", "consistency_check", "relevance_score"]:
            return self._llm_evaluate(row, extractions, config)
        
        return None
    
    def _exact_match(self, row: Union[pd.Series, Dict], extractions: List, config: Dict) -> str:
        """Check exact match"""
    ...
    def _fuzzy_match(self, row: Union[pd.Series, Dict], extractions: List, config: Dict) -> str:
        """Fuzzy matching"""
    ...
    def _llm_evaluate(self, row: Union[pd.Series, Dict], extractions: List, config: Dict) -> str:
