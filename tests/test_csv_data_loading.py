...
def test_csv_load():
    ...
    for idx, row in df.iterrows():
        ...
        assert isinstance(paragraphs, list), f"Paragraphs should be list, got {type(paragraphs)}"
...
def test_config_load():
    ...
    if config_path.exists():
        ...
        print(f"  - Chunking strategy: {config.get('chunking', {}).get('strategy', 'not set')}")
    else:
...
def test_extraction_preview(df):
    ...
    for idx, row in df.iterrows():
        ...
        contract_types = ["MASTER SERVICE AGREEMENT", "NON-DISCLOSURE AGREEMENT", 
                         "STATEMENT OF WORK", "MSA", "NDA", "SOW"]
        found_type = None
        ...
        # Check for dates
        dates = re.findall(r"(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2},\s+\d{4}", all_text)
        if dates:
